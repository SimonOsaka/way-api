...
public class SensitiveFilter implements Serializable {
	...
	public static final SensitiveFilter DEFAULT = new SensitiveFilter(new BufferedReader(
			new InputStreamReader(ClassLoader.getSystemResourceAsStream("sensiWords.txt"),
			...
			if (node != null) {
				...
				for (; node != null; node = node.next) {
					...
					if (node.headTwoCharMix == mix) {
						 ...
						 * 则从“色情电影”开始向前匹配
						 */
						NavigableSet<StringPointer> desSet = node.words
								.headSet(sp.substring(i), true);
						if (desSet != null) {
